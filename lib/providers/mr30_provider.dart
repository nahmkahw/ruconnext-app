...
class MR30Provider extends ChangeNotifier {
  ...
  final MR30Service _service;

  MR30Provider({required MR30Service service}) : _service = service;
  ...
  MR30 _listmr30 = MR30(courseSemester: "", courseYear: "", rECORD: []);
  MR30 get listmr30 => _listmr30;
  ...
  Future<void> fetchData() async {
    print('your service to fetch the data');
    isLoading = true;
    _errormr30 = '';
    try {
      var response = await _service.getAllMR30();
      print(response.courseYear);
      _listmr30 = response;
      _listmr30.rECORD = response.rECORD!.sublist(0, 20);
      print(_listmr30.courseYear);
    } catch (e) {
  ...
  Future<void> loadMoreData() async {
    ...
    var response =
        await _service.getMR30Pagination(_listmr30.rECORD!.length, loadLimit);
    _listmr30.rECORD!.addAll(response.rECORD!);
    notifyListeners();
  }

  void getYearSemesterLatest() async {
    //print('provider:getYearSemesterLatest');
    //await YearSemtesterStorage.removeYearSemester();

    YearSemester yearSemester = await YearSemtesterStorage.getYearSemester();
    isLoading = true;
    _erroryearsemester = '';

    //if (yearSemester.year != "" && yearsemester.semester != "") {
    try {
      final response = await _service.getYearSemesterLatest();
      _yearsemester = response;
      await YearSemtesterStorage.saveYearSemester(_yearsemester);
      //print(_yearsemester.year);
    } catch (e) {
  ...
  void getAllMR30() async {
    isLoading = true;
    _errormr30 = '';
    _mr30.rECORD = _mr30filter.rECORD = [];
  ...
  void getHaveTodayList() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    final mr30 = prefs.getString('mr30') == null ? "" : prefs.getString('mr30');
    ...
    final mr30register = prefs.getString('mr30register') == null
        ? ""
        : prefs.getString('mr30register');
    List<RECORD> listmr30register = [];
    if (mr30register != "") {
  ...
  void getHaveToday() async {
    ...
    //print('storage: ${yearSemester.year} ${yearSemester.semester}');
  ...
  void filterTimeCourseStudy() async {
    //print('call filterTimeCourseStudy');
    initializeDateFormatting('th_TH', null);
    SharedPreferences prefs = await SharedPreferences.getInstance();
    ...
    _havetodayNow = listmr30
        .where((RECORD r) =>
            ...
            (r.courseYear!.toString().contains(yearSemester.year) &&
                r.courseSemester!.toString().contains(yearSemester.semester)) &&
