...
class _MyHomePageState extends State<MyHomePage> with TickerProviderStateMixin {
  ...
  Future<bool> getDataHomeList() async {
  ...
  Future<bool> getDataSchedule() async {
    await Future<dynamic>.delayed(const Duration(milliseconds: 1000));
    return true;
  }
  ...
  Widget build(BuildContext context) {
    //context.read<MR30Provider>().getYearSemesterLatest();
    //context.read<MR30Provider>().getHaveToday();
    //context.watch<MR30Provider>().filterTimeCourseStudy();
    //context.read<RegisterProvider>().getHaveTodayRegister();
    var authen = context.watch<AuthenProvider>();
    //authen.getProfile();
    //print('Role: ${authen.profile.studentCode}');
    switch (authen.role) {
      case "Doctor":
                      ...
                      return authen.role != "Bachelor"
                          ? Card(
                              child: Padding(
                                ...
                                child: ListTile(
                                  title: Text(
                                    '${mr30.havetodayNow[index].courseNo} (${mr30.havetodayNow[index].courseYear}/${mr30.havetodayNow[index].courseSemester})',
                                    style: TextStyle(
                                      fontSize: 12,
                                    ),
                                  ),
